...
class CIFAR10(data.Dataset):
    ...
    def __init__(self, root, split='train+test',
                 ...
                 download=False, target_list = range(5), noisy_labels=None):
...
def CIFAR10Data(root, split='train', aug=None, target_list=range(5), noisy_labels=None):
    if aug==None:
    ...
    dataset = CIFAR10(root=root, split=split, transform=transform, target_list=target_list, noisy_labels=noisy_labels)
    return dataset

def CIFAR10Loader(root, batch_size, split='train', num_workers=2,  aug=None, shuffle=True, target_list=range(5), noisy_labels=None):
    dataset = CIFAR10Data(root, split, aug,target_list, noisy_labels=noisy_labels)
    loader = data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers)
    return loader

def CIFAR10LoaderMix(root, batch_size, split='train',num_workers=2, aug=None, shuffle=True, labeled_list=range(5), unlabeled_list=range(5, 10), new_labels=None, noisy_labels=None):
    if aug==None:
        transform = transforms.Compose([
            transforms.ToTensor(),
            transforms.Normalize((0.4914, 0.4822, 0.4465), (0.2023, 0.1994, 0.2010)),
    ...
    dataset_labeled = CIFAR10(root=root, split=split, transform=transform, target_list=labeled_list, noisy_labels=noisy_labels)
    dataset_unlabeled = CIFAR10(root=root, split=split, transform=transform, target_list=unlabeled_list, noisy_labels=noisy_labels)
